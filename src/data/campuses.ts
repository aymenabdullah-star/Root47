...
export function generateCampuses(): Campus[] {
  ...
  CITIES.forEach((city, cityIdx) => {
    ...
    for (let i = 0; i < quota; i++) {
      ...
      let visibility = 5 + Math.round(random() * 25); // 5 - 30 km
      let uvIndex = 4 + Math.round(random() * 8); // 4 - 12

      // Make certain cities have specific weather traits
      if (city.name === 'Lahore') {
        visibility = 1 + Math.round(random() * 3); // Smoggy/low vis
      ...
      const weather: WeatherTelemetry = {
        ...
        windSpeed,
        uvIndex,
        cloudCover: 10 + Math.round(random() * 70),
        feelsLike: temp + Math.round((humidity - 40) / 10) - Math.round(windSpeed / 10),
        visibility,
        weatherCondition: chanceOfRain > 60 ? 'Rain' : visibility < 3 ? 'Haze' : 'Clear',
      };

      // Set operational status by evaluating initial severity rules:
      // Category A (Severe): Temp > 43°C OR Visibility < 1km OR Rain > 85% + Humidity > 85%
      // Category B (Caution): Temp > 39°C OR Visibility < 3km OR Rain > 70%
      // Safe: Operational
      let operationalStatus: 'Operational' | 'Caution' | 'Severe Disruption' = 'Operational';
      if (weather.temp > 43 || (weather.visibility ?? 10) < 1 || (weather.chanceOfRain > 85 && weather.humidity > 85)) {
        operationalStatus = 'Severe Disruption';
      } else if (weather.temp > 39 || (weather.visibility ?? 10) < 3 || weather.chanceOfRain > 70) {
