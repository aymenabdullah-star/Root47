...
function getWeatherIcon(campus: Campus) {
  ...
  if (w.visibility < 2) return <Activity className="w-5 h-5 text-amber-500" />;
...
export default function App() {
  ...
  const [campusListLimit, setCampusListLimit] = useState<number>(50);
  ...
  const [disasterPresetsExpanded, setDisasterPresetsExpanded] = useState<boolean>(false);
  ...
  const [overrideVisibility, setOverrideVisibility] = useState<number>(5);
  ...
  const [crowdVisibility, setCrowdVisibility] = useState<number>(5);
  ...
  const handleDraftAlertWithGemini = async (campus: Campus) => {
    ...
    } else if (campus.weather.visibility < 3) {
      hazardType = 'Low Visibility / Haze';
      hazardDesc = `Visibility reduced to ${campus.weather.visibility}km. Possible smog, fog or dust haze conditions.`;
    }
    ...
    try {
      const res = await fetch('/api/gemini/draft-alert', {
        ...
        body: JSON.stringify({
          ...
          visibility: campus.weather.visibility,
          condition: campus.weather.weatherCondition,
  ...
  const handleBroadcastCommit = async () => {
    ...
    try {
      ...
      if (emailDeliveryActive) {
        try {
          const emailRes = await fetch('/api/broadcast/email', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              subject: newAlert.title,
              emailBody: draftResult?.emailBody || newAlert.message,
              campusName: campus.name,
              campusId: campus.branchId,
              region: campus.region,
            })
          ...
          emailStatusMsg = ` · Email error: ${emailErr.message}`;
        }
      }

      setGlobalBannerText({ 
        text: `Safety dispatch broadcasted for ${campus.name}.${emailStatusMsg}`, 
        type: 'success' 
      });
  ...
  const filteredCampuses = useMemo(() => {
    setCampusListLimit(50); // reset pagination on filter change
  ...
  useEffect(() => {
    if (selectedCampus) {
      ...
      setOverrideVisibility(selectedCampus.weather.visibility ?? 10);
  ...
  return (
    <div className={`min-h-screen ${clBackground} font-sans antialiased transition-colors duration-300`}>
        ...
        <div className="max-w-[1600px] mx-auto px-4 lg:px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
            ...
            <button 
              ...
              title="Fetch live weather from WeatherWalay API for all campuses."
              id="btn-live-sync"
            >
      ...
      <section className={`transition-colors duration-300 ${
        ...
        <div className="max-w-[1600px] mx-auto px-4 lg:px-6 py-3 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1.5">
              <span className={`text-[11px] font-mono ${isMidnight || !isMinimalist ? 'text-slate-400' : 'text-bss-navy-dark/70'}`}>Campuses:</span>
              <span className={`font-black text-sm px-2.5 py-0.5 rounded-md ${
                isMidnight ? 'bg-slate-900 text-white border border-slate-800' : isMinimalist ? 'bg-white text-bss-navy' : 'bg-bss-navy text-bss-gold border border-bss-gold/30'
              }`}>{stats.total}</span>
            </div>
            <div className={`h-4 w-px ${isMidnight ? 'bg-slate-800' : 'bg-white/20'}`}></div>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 bg-rose-500 rounded-full animate-pulse shrink-0"></span>
              <span className={`text-[11px] font-mono ${isMidnight || !isMinimalist ? 'text-slate-400' : 'text-bss-navy-dark/70'}`}>Severe:</span>
              <span className="text-rose-400 font-black text-sm tabular-nums">{stats.severe}</span>
            </div>
            <div className={`h-4 w-px ${isMidnight ? 'bg-slate-800' : 'bg-white/20'}`}></div>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 bg-amber-400 rounded-full shrink-0"></span>
              <span className={`text-[11px] font-mono ${isMidnight || !isMinimalist ? 'text-slate-400' : 'text-bss-navy-dark/70'}`}>Caution:</span>
              <span className={`font-black text-sm tabular-nums ${isMidnight || !isMinimalist ? 'text-amber-400' : 'text-amber-500'}`}>{stats.caution}</span>
            </div>
            <div className={`h-4 w-px ${isMidnight ? 'bg-slate-800' : 'bg-white/20'}`}></div>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 bg-emerald-400 rounded-full shrink-0"></span>
              <span className={`text-[11px] font-mono ${isMidnight || !isMinimalist ? 'text-slate-400' : 'text-bss-navy-dark/70'}`}>OK:</span>
              <span className="text-emerald-400 font-black text-sm tabular-nums">{stats.operational}</span>
            </div>
            {stats.geofenced > 0 && (
              <div className={`flex items-center gap-1.5 pl-2 border-l ${
                ...
                <span className="text-rose-400 font-bold font-sans text-[11px]">Geofenced: {stats.geofenced}</span>
              </div>
            )}
          </div>

          <div className="flex items-center gap-2">
            <span className={`text-[11px] font-mono ${isMidnight || !isMinimalist ? 'text-bss-gold/80' : 'text-bss-navy'}`}>Logged in:</span>
            <span className={`font-semibold text-[11px] ${isMidnight || !isMinimalist ? 'text-white' : 'text-slate-900'}`}>{currentProfile.name}</span>
      ...
      <main className="max-w-[1600px] mx-auto p-4 lg:p-6">
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          <div className={`xl:col-span-8 space-y-6 ${!isHandsetOpen ? 'xl:col-span-12' : ''} transition-all duration-300`}>
            ...
            <div className={`flex items-center justify-between border-b pb-0 ${isMidnight ? 'border-slate-800' : isMinimalist ? 'border-bss-gold/20' : 'border-slate-200'}`}>
              <nav className="flex gap-1">
                <button
                  onClick={() => setActiveTab('telemetry')}
                  className={`px-4 pb-3 pt-1 text-sm font-semibold border-b-2 transition-all relative ${
                    activeTab === 'telemetry' 
                      ? isMidnight
                      ...
                      : isMidnight 
                        ...
                        : 'border-transparent text-slate-500 hover:text-bss-navy hover:border-slate-200'
                  }`}
                  id="tab-telemetry"
                >
              ...
              <div className={`text-[10px] font-mono font-bold uppercase tracking-widest px-2 py-1 rounded ${isMidnight ? 'text-bss-gold bg-bss-gold/10' : isMinimalist ? 'text-bss-navy bg-bss-navy/10' : 'text-bss-navy bg-bss-navy/5'}`}>
                BMW LIVE
              </div>
            </div>
            ...
            {activeTab === 'telemetry' && (
              <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="flex items-end justify-end">
                    ...
                    >
                      ...
                      Clear Filter Metrics
                                ...
                                {/* DISASTER PRESETS PANEL (GEOFENCING CONTROL) — collapsible */}
                <div className={`rounded-xl border transition-all duration-300 overflow-hidden ${
                  isMidnight 
                    ? 'bg-amber-950/20 border-bss-gold/30' 
                    : isMinimalist
                      ? 'bg-bss-gold-light/30 border-bss-gold-dark/30'
                      : 'bg-amber-50/60 border-amber-200'
                }`}>
                  {/* Collapse toggle header */}
                  <button
                    onClick={() => setDisasterPresetsExpanded(prev => !prev)}
                    className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left"
                  >
                    <div className="flex items-center gap-2.5">
                      <ShieldAlert className={`w-4.5 h-4.5 shrink-0 ${isMidnight ? 'text-bss-gold' : 'text-amber-600'}`} />
                      <div>
                        <span className={`text-sm font-black leading-tight ${isMidnight ? 'text-bss-gold' : 'text-amber-900'}`}>
                          Safety Drill Presets
                        </span>
                        <span className={`hidden sm:inline ml-2 text-[11px] font-normal ${isMidnight ? 'text-slate-400' : 'text-amber-700'}`}>
                          — 12 km geofencing simulation for 3 disaster scenarios
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {Object.values(disasterPresetsActive).filter(Boolean).length > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black bg-rose-500 text-white animate-pulse">
                          {Object.values(disasterPresetsActive).filter(Boolean).length} ACTIVE
                        </span>
                      )}
                      ...
                      <ChevronRight className={`w-4 h-4 transition-transform duration-200 ${disasterPresetsExpanded ? 'rotate-90' : ''} ${isMidnight ? 'text-slate-400' : 'text-amber-600'}`} />
                    </div>
                  </button>

                  {/* Expanded content */}
                  {disasterPresetsExpanded && (
                    <div className={`px-4 pb-4 border-t ${isMidnight ? 'border-bss-gold/20' : 'border-amber-200/60'}`}>
                      <p className={`text-[11px] pt-3 pb-3 ${isMidnight ? 'text-slate-400' : 'text-amber-700'}`}>
                        Activate a regional scenario to geofence all campuses within 12 km into Category A Severe status and trigger automated safety broadcasts.
                      </p>
                              ...
                              <div className="flex items-start justify-between gap-2 mb-2">
                                <div>
                                  <div className="flex items-center gap-1.5 mb-0.5">
                                    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isActive ? 'bg-rose-500 animate-pulse' : 'bg-slate-400'}`}></span>
                                    <h4 className={`text-xs font-bold leading-tight ${isMidnight ? 'text-white' : 'text-slate-900'}`}>{preset.city} — {preset.hazardType}</h4>
                                  </div>
                                  <p className={`text-[10px] leading-snug line-clamp-2 ${isMidnight ? 'text-slate-400' : 'text-slate-500'}`}>{preset.description}</p>
                                </div>
                              </div>
                              <div className="flex items-center justify-between gap-2">
                                {isActive ? (
                                  <div className={`text-[10px] font-bold font-mono flex items-center gap-1 px-2 py-1 rounded flex-1 ${
                                    isMidnight ? 'text-rose-400 bg-rose-950/40' : 'text-rose-700 bg-rose-100'
                                  }`}>
                                    <ShieldCheck className="w-3 h-3 shrink-0" />
                                    12 KM geofence active
                                  </div>
                                ) : <div className="flex-1" />}
                                <button
                                  onClick={() => handleDisasterToggle(preset.id, isActive)}
                                  className={`px-2.5 py-1 rounded-md text-[10px] font-black shrink-0 transition-colors ${
                                    isActive 
                ...
                {/* NATIONAL BSS MICRO-STATIONS CLUSTER MAP */}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <h3 className={`text-sm font-bold ${isMidnight ? 'text-white' : 'text-slate-800'}`}>
                      BSS Branch Operations — <span className={isMidnight ? 'text-bss-gold' : 'text-bss-navy'}>{filteredCampuses.length} campuses</span> matching
                    </h3>
                    <div className={`text-xs font-mono ${isMidnight ? 'text-slate-400' : 'text-slate-500'}`}>
                      {stats.severe > 0 && <span className="text-rose-500 font-bold mr-3">⚠ {stats.severe} Severe</span>}
                      {stats.caution > 0 && <span className="text-amber-500 font-bold mr-3">⚡ {stats.caution} Caution</span>}
                      <span className="text-emerald-500 font-bold">{stats.operational} Operational</span>
                    </div>
                  </div>
                  ...
                  {filteredCampuses.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {filteredCampuses.slice(0, campusListLimit).map(campus => {
                        const isSelected = selectedCampusId === campus.branchId;
                        return (
                          <div 
                            key={campus.branchId}
                            onClick={() => setSelectedCampusId(campus.branchId)}
                            className={`rounded-xl border-l-4 border cursor-pointer transition-all ${
                              campus.operationalStatus === 'Severe Disruption' ? 'border-l-rose-500' :
                              campus.operationalStatus === 'Caution' ? 'border-l-amber-400' :
                              'border-l-emerald-400'
                            } ${
                              isSelected 
                                ? isMidnight
                                  ? 'bg-slate-900 border-bss-gold ring-1 ring-bss-gold/30 shadow-md' 
                                  : isMinimalist
                                    ? 'bg-bss-gold-light/40 border-bss-navy ring-1 ring-bss-navy/20'
                                    : 'bg-indigo-50/40 border-bss-navy ring-1 ring-bss-gold/50 shadow-md' 
                                : isMidnight 
                                  ? 'bg-slate-900/50 border-slate-700 hover:border-slate-600 hover:shadow-md shadow-sm' 
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap mb-1.5">
                                  ...
                                  <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded uppercase ${
                                    campus.region === 'Centre' ? 'bg-indigo-100 text-indigo-700' :
                                    campus.region === 'South' ? 'bg-emerald-100 text-emerald-700' :
                              ...
                              <div className="flex flex-col items-end gap-1.5 shrink-0">
                                ...
                                <div className={`flex items-center gap-1.5 text-sm font-mono font-extrabold ${isMidnight ? 'text-slate-100' : 'text-slate-800'}`}>
                            ...
                            <div className={`grid grid-cols-5 gap-1 mt-4 pt-3.5 border-t text-[10px] font-mono text-center ${
                              isMidnight ? 'border-slate-700/60 text-slate-400' : 'border-slate-100 text-slate-500'
                            }`}>
                              <div className="space-y-0.5">
                                <span className={`block text-[9px] font-semibold uppercase tracking-wide ${isMidnight ? 'text-slate-500' : 'text-slate-400'}`}>Feels</span>
                                <span className={`font-bold text-xs block ${campus.weather.feelsLike > 43 ? 'text-rose-500' : isMidnight ? 'text-slate-200' : 'text-slate-800'}`}>
                                  {campus.weather.feelsLike}°
                                </span>
                              </div>
                              <div className="space-y-0.5">
                                <span className={`block text-[9px] font-semibold uppercase tracking-wide ${isMidnight ? 'text-slate-500' : 'text-slate-400'}`}>Rain</span>
                                <span className={`font-bold text-xs block ${isMidnight ? 'text-slate-200' : 'text-slate-800'}`}>{campus.weather.chanceOfRain}%</span>
                              </div>
                              <div className="space-y-0.5">
                                <span className={`block text-[9px] font-semibold uppercase tracking-wide ${isMidnight ? 'text-slate-500' : 'text-slate-400'}`}>Hum</span>
                                <span className={`font-bold text-xs block ${isMidnight ? 'text-slate-200' : 'text-slate-800'}`}>{campus.weather.humidity}%</span>
                              </div>
                              <div className="space-y-0.5">
                                <span className={`block text-[9px] font-semibold uppercase tracking-wide ${isMidnight ? 'text-slate-500' : 'text-slate-400'}`}>Wind</span>
                                <span className={`font-bold text-xs block ${isMidnight ? 'text-slate-200' : 'text-slate-800'}`}>{campus.weather.windSpeed}</span>
                              </div>
                              <div className="space-y-0.5">
                                <span className={`block text-[9px] font-semibold uppercase tracking-wide ${isMidnight ? 'text-slate-500' : 'text-slate-400'}`}>Vis</span>
                                <span className={`font-bold text-xs block ${campus.weather.visibility < 3 ? 'text-amber-500' : isMidnight ? 'text-slate-200' : 'text-slate-800'}`}>{campus.weather.visibility}km</span>
                              </div>
                            </div>
                            {campus.weather.weatherCondition && campus.weather.weatherCondition !== 'Clear' && (
                              <div className={`mt-2.5 text-[10px] font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1.5 ${isMidnight ? 'bg-slate-800/80 text-slate-300' : 'bg-slate-50 text-slate-600 border border-slate-100'}`}>
                                <Cloud className="w-3 h-3 flex-shrink-0 opacity-70" />
                                {campus.weather.weatherCondition} · Cloud cover {campus.weather.cloudCover}%
                              </div>
                            )}

                            {campus.manualOverride && (
                              <div className="mt-3 bg-indigo-50 border border-indigo-100 text-[10px] text-indigo-800 px-3 py-2 rounded-lg flex items-start gap-1.5">
                                <Sliders className="w-3.5 h-3.5 text-indigo-500 flex-shrink-0 mt-0.5" />
                                <div>
                                  <span className="font-bold block">Manual Override Active</span>
                                  <span className="text-indigo-600 italic">"{campus.overrideReason}"</span>
                                </div>
                              </div>
                            )}

                            {campus.hazardEffectId && (
                              <div className="mt-3 bg-rose-50 border border-rose-200 text-[10px] text-rose-800 px-3 py-2 rounded-lg flex items-center gap-1.5 font-bold">
                                <ShieldAlert className="w-3.5 h-3.5 text-rose-500 flex-shrink-0 animate-pulse" />
                  ...
                  {filteredCampuses.length > campusListLimit && (
                    <div className="text-center py-4">
                      <button
                        onClick={() => setCampusListLimit(prev => prev + 50)}
                        className={`px-6 py-2.5 rounded-lg text-sm font-bold border transition-all ${
                          isMidnight
                            ? 'bg-slate-900 border-bss-gold/40 text-bss-gold hover:bg-slate-800'
                            : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50 shadow-sm'
                        }`}
                      >
                        Load More Campuses ({filteredCampuses.length - campusListLimit} remaining)
                      </button>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-3xs space-y-4">
                    ...
                    <form onSubmit={handleCrowdReportSubmit} className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[10px] font-extrabold uppercase text-slate-500 mb-1">Visibility (km)</label>
                          <input
                            type="number"
                            min="0"
                            max="50"
                            step="0.5"
                            value={crowdVisibility}
                            onChange={(e) => setCrowdVisibility(parseFloat(e.target.value) || 5)}
                            className="bg-slate-50 border border-slate-200 py-2 px-3 rounded-lg text-xs outline-none w-full"
                          />
                        </div>
                        <div>
                        ...
                        incidentReports.map(report => (
                          <div key={report.id} className="p-3.5 bg-slate-50 rounded-lg border border-slate-200 space-y-2">
                            ...
                            <div className="flex items-center gap-3 font-mono text-[9px] text-slate-500 bg-slate-100 px-2 py-1 rounded">
                              ...
                              <span>Vis: {(report.reportedWeather as any).visibility ?? 'N/A'}km</span>
                ...
                <div className="p-4 space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center text-slate-800">
                      ...
                      <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                        <span className="block text-[9px] text-slate-400 font-semibold mb-0.5">VISIBILITY</span>
                        <span className={`text-sm font-bold block ${selectedCampus.weather.visibility < 3 ? 'text-amber-600' : ''}`}>{selectedCampus.weather.visibility ?? 'N/A'} km</span>
                        <span className="text-[8px] text-slate-400 block font-mono">km range</span>
                      </div>
                      <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  ...
                  <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          <label className="block text-[9px] text-slate-500 font-bold mb-0.5">Visibility (km) Override</label>
                          <input 
                            type="number"
                            value={overrideVisibility} 
                            onChange={(e) => setOverrideVisibility(parseFloat(e.target.value) || 5)}
                            min="0" 
                            max="50"
                            step="0.5"
                            disabled={currentProfile.role !== 'super_admin' && (currentProfile.role === 'regional_admin' ? currentProfile.region !== selectedCampus.region : true)}
